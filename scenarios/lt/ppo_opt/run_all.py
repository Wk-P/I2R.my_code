...
class P6Callback(BaseCallback):
    def __init__(self):
        ...
        # NOTE: these count best-fit REPAIR triggers, not unrepaired
        # violations in the delivered placement (which is always 0 by
        # construction -- action=repaired replaces the action before it's
        # applied to env state). See paper_contents docs for the distinction.
...
def plot_training_curve(cb: P6Callback, ilp_ar: float, outdir: Path, scenario_name: str):
    ...
    # plot_training_curve() comment for rationale. NOTE: for this algorithm
    # specifically, the violation-rate panels are REPAIR-TRIGGER rates, not
    # unrepaired violations in the delivered placement (which is always 0 by
    # construction) -- see P6Callback's comment.
    ...
    ax3.plot(ts[off_cap:off_cap+len(sm_cap)], sm_cap, color="tomato", linewidth=2,
             label=f"cap repair-trigger rate (smoothed w={C.SMOOTH_W})")
    ...
    ax4.plot(ts[off_conf:off_conf+len(sm_conf)], sm_conf, color="darkorange", linewidth=2,
             label=f"privacy (conflict) repair-trigger rate (smoothed w={C.SMOOTH_W})")
...
def plot_comparison(ilp_ar, ppo_res, cap_viol_rate, conflict_viol_rate, outdir: Path, scenario_name: str):
    ...
    # plot_comparison() comment for rationale. NOTE: cap_viol_rate/
    # conflict_viol_rate passed in here are REPAIR-TRIGGER rates for this
    # algorithm, not unrepaired violations in the delivered placement (always
    # 0 by construction) -- kept consistent with what summary.csv reports
    # under the same column names for this algorithm.
    algo_label = "PPO\n(P6, repair)"
    success_rate = float(np.mean(ppo_res["success"]))

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(11, 5))
    fig.suptitle(f"P2(ILP) vs P6(PPO+repair) - {scenario_name}", fontsize=13, fontweight="bold")
...
def main():
    ...
    # see lt/ppo/run_all.py's comment for why best-of-N was dropped.
    def ppo_policy(obs):
