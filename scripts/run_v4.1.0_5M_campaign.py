#!/usr/bin/env python3
"""v4.1.0 supplementary campaign: lt/eq/gt x 6-algorithm x 3-seed at 5M steps.
...
Same step budget as the frozen v4.0.0 5M/5-seed campaign, but only 3 seeds
instead of 5 -- this matches the training-budget confound out of the
comparison (unlike the earlier 2M-step v4.1.0 run) while keeping compute
manageable. Still not a full replacement for the frozen results/add_states
data (fewer seeds), so treat it as a closer-but-not-identical ablation.
...
written to scripts/logs/v4.1.0_5M_campaign/manifest.json so the report
...
Usage:
    nohup .venv/bin/python scripts/run_v4.1.0_5M_campaign.py > scripts/logs/run_v4.1.0_5M_campaign_driver.log 2>&1 &
    disown
"""
import json
import subprocess
...
LOG_DIR = PROJECT_ROOT / "scripts" / "logs" / "v4.1.0_5M_campaign"
