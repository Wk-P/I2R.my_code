#!/usr/bin/env python3
"""v4.1.0.2 targeted verification: gt x ppo_lagrangian ONLY x 3-seed at 5M steps.

Purpose: after v4.1.0 (full formula) and v4.1.0.1 (penalty-only, dropped
match_gain) BOTH regressed lt hard versus v4.0.0 (success_rate ~0.52-0.54 vs
0.81) with no meaningful difference between them, ppo_lagrangian's per-step
reward was reverted to 0.0 in all 3 scenarios (v4.1.0.2, matching v4.0.0's
original behaviour) -- see paper_contents/v4.1.0_changelog.md for the full
ablation history.

lt and eq are now byte-for-byte equivalent to v4.0.0's reward behaviour, so
their v4.0.0 frozen results can be reused directly -- no rerun needed. gt is
the ONE exception: its step()-internal capacity redirect (using
action_masks(), inconsistent with lt/eq which never had one) was removed
back in v4.1.0 and stays removed in v4.1.0.2 as a separate, deliberate
correctness fix -- so v4.1.0.2's gt is NOT identical to v4.0.0's gt (which
still had that redirect), even though the reward formula itself now matches.
This script reruns ONLY gt/ppo_lagrangian to get a clean number for that one
new combination (redirect removed + reward=0.0), which has never been tested
before.
...
written to scripts/logs/v4.1.0.2_gt_ppo_lagrangian_5M/manifest.json so the report
...
Usage:
    nohup .venv/bin/python scripts/run_v4.1.0.2_gt_ppo_lagrangian_5M.py > scripts/logs/run_v4.1.0.2_gt_ppo_lagrangian_5M_driver.log 2>&1 &
    disown
"""
import json
import subprocess
...
LOG_DIR = PROJECT_ROOT / "scripts" / "logs" / "v4.1.0.2_gt_ppo_lagrangian_5M"
