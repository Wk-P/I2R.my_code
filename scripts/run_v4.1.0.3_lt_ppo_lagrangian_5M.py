#!/usr/bin/env python3
"""v4.1.0.3 targeted verification: lt x ppo_lagrangian ONLY x 3-seed at 5M steps.

Purpose: v4.1.0.2 reverted ppo_lagrangian's non-terminal reward to 0.0
everywhere after v4.1.0/v4.1.0.1 both regressed lt hard (success_rate
~0.52-0.54 vs v4.0.0's 0.81). That left ppo_lagrangian's lambda dual-ascent
mechanism running but completely disconnected from training -- the algorithm
no longer does anything "Lagrangian" in practice. Root-caused afterwards
(see paper_contents/v4.1.0_changelog.md's v4.1.0.3 section): LAMBDA_TARGET=0
means dual ascent's update term is never negative, so lambda is monotonically
driven toward LAMBDA_MAX over training -- the raw penalty -(lambda+
base_penalty)*c_t therefore drifts in magnitude by >10x from early to late
training, a nonstationary reward target PPO's GAE/value function can't
track. v4.1.0.3 rescales the penalty into a fixed [0, penalty_ceiling] range
(lambda's own dual-ascent trajectory is untouched) and lengthens
LAMBDA_UPDATE_WINDOW so the target moves more slowly.

This script validates ONLY lt (the scenario that regressed) to check whether
the fix recovers performance toward v4.0.0's reference (success_rate ~0.81,
conflict_viol ~0.18) before deciding whether to port the same fix to eq/gt
